/* --- AURA V83.2 (LEGACY SAFE MODE - SHELLY SYNC) --- */
...
var CONFIG = {
    ...
    shellyIP: localStorage.getItem('aura_shelly_ip') || '' // NEU
};

/* --- DATENBANKEN (UNVERÄNDERT) --- */
...
function startApp() {
    ...
    runClock(); 
    loadData(); 
    updateShelly(); // NEU: Shelly Initialisierung
    checkStatus(); 
    initBatteryGuard(); 

    setInterval(runClock, 1000);       
    setInterval(loadData, 600000);     
    setInterval(updateShelly, 60000);  // NEU: Shelly Update Zyklus (1 Min)
...
function initMenuValues() {
    ...
    var sIP = document.getElementById('inp-shelly-ip'); if(sIP) sIP.value = CONFIG.shellyIP; // NEU
}
...
function saveSettings() {
    ...
    var sIP = document.getElementById('inp-shelly-ip').value; // NEU
    ...
    localStorage.setItem('aura_shelly_ip', sIP); CONFIG.shellyIP = sIP; // NEU
    
    closeMenu(); 
    loadData();
    updateShelly(); // Sofort-Update nach dem Speichern
}
...
function updateShelly() {
    ...
    xhr.onerror = function() {
        document.getElementById('shelly-temp').innerText = "ERR";
    };
    
    xhr.send();
}

/* --- RESTLICHE ENGINE (UNVERÄNDERT) --- */
function runClock() {
    var now = new Date();
...
function loadData() {
    ...
    fetch(urlCurrent).then(function(r) { return r.json(); }).then(function(current) {
    ...
    }).then(function(r) { return r.json(); }).then(function(forecast) {
        globalForecastCache = forecast;
        renderForecast(forecast);
        return loadTicker();
    }).then(function() { hideLoader(); }).catch(function(e) {
...
function loadTicker() {
    var tickerContent = "+++ AURA WELT-WETTER +++ ";
    var requests = WORLD_CITIES.map(function(city) { 
        return fetch('https://api.openweathermap.org/data/2.5/weather?q=' + city.n + '&appid=' + CONFIG.apiKey + '&units=metric')
        .then(function(r){ return r.json(); }).then(function(d){ d.flag = city.f; return d; }).catch(function(){ return null; });
    });
